...
Capabilities:
  ...
  - Streams graph execution with full step observability.
...
class ChatPipeline:
    ...
    async def send(self, message: str) -> AsyncGenerator[dict, None]:
        ...
        # trace_id: new turns get one from create_initial_state;
        # continued turns get theirs from state after the first event.
        ...
        # turn (input_gate, chat_react, sub-agents, retries).
        ...
        try:
            async for event in self._graph.astream_events(
                ...
                # --- LLM call start: record wall-clock for duration ---
                elif kind == "on_chat_model_start":
                    if run_id:
                        _last_chat_model_start[run_id] = time.perf_counter()

                # --- LLM call end: capture usage + duration ---
                ...
                # --- Streaming tokens from chat agent ---
                elif kind == "on_chat_model_stream" and node in (
                    "chat_react", "chat_react_retry",
                ):
    ...
    async def _persist_exchange(
        ...
        *,
        ...
        Authenticated-only. One connection shared between conversation
        log writes and the cost tracker flush — same try/finally, so
        no connection leaks on partial failure.
        """
        if not self.user_id:
            return
        ...
        try:
            ...
            try:
                ...
                # 2. LLM usage (non-fatal if it fails)
