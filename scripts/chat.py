...
Event types yielded by send():
  ...
  done         — turn complete with timing + tool stats
...
class ChatPipeline:
    ...
    async def send(self, message: str) -> AsyncGenerator[dict, None]:
        ...
        try:
            ...
            if graph_state.next:
                ...
                await self._persist_exchange(message, final_content)
    ...
    async def _persist_exchange(self, user_msg: str, assistant_msg: str):
        if not self.user_id:
            return
