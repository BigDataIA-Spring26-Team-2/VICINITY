...
Tag partitioning reads livability_tags from the same config.

Usage:
    python -m app.pipelines.ingest_google_news --preference-tag safety --dry-run
    python -m app.pipelines.ingest_google_news --tags safety noise korean_food
    python -m app.pipelines.ingest_google_news --category livability --dry-run
    python -m app.pipelines.ingest_google_news --category lifestyle
    python -m app.pipelines.ingest_google_news --dry-run
        ...
        for attempt in range(1, self._max_attempts + 1):
            try:
                ...
                if len(html) < 1000:
                    ...
                    self._log.warning("page_too_short", url=url[:80],
                ...
                wait = min(self._backoff_base ** attempt, self._backoff_max)
                self._log.warning("fetch_failed", url=url[:80],
...
class NewsExtractor:
    """RSS fetch → regex XML parse → Playwright article extraction."""
    ...
    def fetch_rss(self, query: str) -> list[dict]:
        ...
        for raw in raw_items[:self._max_articles]:
            ...
            if entry.get("source") in self._blocked:
                self._log.debug("blocked_source_skipped", source=entry["source"])
                continue
            entries.append(entry)
        self._log.info("rss_parsed", query=query[:50], raw=len(raw_items), accepted=len(entries))
        return entries

    @staticmethod
    def _parse_rss_item(raw: str) -> dict:
        title_m = re.search(r"<title>(.*?)</title>", raw)
        link_m = re.search(r"(https://news\.google\.com/rss/articles/[^<\s\"']+)", raw)
    ...
    def extract_article(self, entry: dict) -> dict | None:
        ...
        if not text:
            self._log.warning("extraction_failed", source=entry.get("source"), url=final_url[:80])
            return None
        published_iso = self._parse_pub_date(entry.get("published", ""))
        return {
            **entry, "final_url": final_url, "title": title,
            "article_text": text, "extraction_method": method,
            "paragraph_count": para_count, "text_len": len(text),
...
class NewsClassifier:
    """Two-stage LLM classification: batch filter → deep per-article analysis."""

    def __init__(self, cursor, pipeline_run_id: str):
        config = load_classification()
        prompts = config.get("prompts", {})
        if "google_news_filter" not in prompts:
            raise ValueError("Missing 'google_news_filter' prompt in classification.yml")
        if "google_news" not in prompts:
            raise ValueError("Missing 'google_news' prompt in classification.yml")
    ...
    def stage1_filter(self, articles: list[dict], preference_tag: str,
                      query: str) -> list[dict]:
        if not articles:
            return []
        items = [
        ...
        user_prompt = json.dumps({"preference_tag": preference_tag, "query": query, "articles": items})
        try:
    ...
    def stage2_classify(self, article: dict, preference_tag: str,
                        query: str) -> dict | None:
        ...
        try:
            ...
            self._log.error("stage2_failed", title=article.get("title", "")[:60], error=str(exc))
            return None
...
class GoogleNewsPipeline(BasePipeline):
    """Orchestrates RSS fetch → extract → dedup → filter → classify → flush per query."""
    ...
    def _run_tag(self, tag: str, args: argparse.Namespace) -> PipelineRunResult:
        ...
        for qi, q in enumerate(queries):
            ...
            self.log.info("query_rss", query=q, raw=len(entries), unique=len(unique))
            ...
            # Extract full article text (Playwright — needed to get final_url for dedup)
            articles: list[dict] = []
            ...
            # Skip articles already in Snowflake from prior runs
            novel: list[dict] = []
            for a in articles:
                final_url = a.get("final_url") or a.get("link", "")
                sid = hashlib.sha256(f"google_news:{final_url}:{tag}".encode()).hexdigest()[:64]
                if sid not in seen_signals:
                    novel.append(a)

            skipped_seen = len(articles) - len(novel)
            if skipped_seen:
                self.log.info("skipped_seen", query=q, skipped=skipped_seen)

            if not novel:
                self.log.info("query_no_new_articles", query=q)
                total_extracted += len(unique)
                total_skipped += skipped_seen
                continue

            scored = classifier.stage1_filter(novel, tag, q)
            relevant = [a for a in scored if a.get("_stage1_score", 0) >= s1_thresh]
            ...
            for article in relevant:
                classification = classifier.stage2_classify(article, tag, q)
                if not classification:
                    self.record_error(record_key=article.get("final_url", "")[:100],
            ...
            if args.dry_run:
                ...
                for r in transformed[:2]:
                    self.log.info("sample", title=r["title"][:80],
    ...
    def _transform(self, article: dict, preference_tag: str) -> dict | None:
        ...
        signal_id = hashlib.sha256(f"google_news:{final_url}:{preference_tag}".encode()).hexdigest()[:64]
        content_hash = hashlib.sha256(f"{title}|{article_text}".encode()).hexdigest()
        published_iso = article.get("published_iso")

        return {
            "signal_id": signal_id, "signal_source": "google_news",
            "source_native_id": final_url[:100], "preference_tag": preference_tag,
            "title": title, "snippet_text": narrative,
            "raw_thread_text": article_text, "url": final_url,
